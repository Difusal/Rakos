...
void TutorialState::moveObject(LivingBeingSon *obj)
{
...
{
	/* loading map */
	LoadMap(TutorialWorldMap, worldMap);
...
bool TutorialState::Update(ALLEGRO_EVENT * ev)
{
	...
	if (prev_size != movingCreaturesAndNpcs.size())
	{
		allMovingStuff.clear();
		allMovingStuff.push_back(player);
		for (LivingBeing* obj : movingCreaturesAndNpcs)
	...
	if (ev->type == ALLEGRO_EVENT_TIMER)
	{
		if (ev->timer.source == StateControl::GetInstance()->GetTimer(0))
		{
			...
			for (unsigned int i = 0; i < movingCreaturesAndNpcs.size(); i++)
				updateLivingBeingsCollisions(player, movingCreaturesAndNpcs[i]);
		...
		if (ev->timer.source == StateControl::GetInstance()->GetTimer(1))
			for (LivingBeing* obj : allMovingStuff)
		...
		sort(allMovingStuff.begin(), allMovingStuff.end(), [](LivingBeing *a, LivingBeing *b) { return a->getY() < b->getY(); });
...
void TutorialState::Draw()
{
	...
	for(unsigned int i = 0; i < worldMap.size(); i++)
	{
		for(unsigned int j = 0; j < worldMap[i].size(); j++)
		{
			...
			{ al_draw_bitmap_region(StateControl::GetInstance()->GetTileSet(), worldMap[i][j] * WorldBlockSize, 10, WorldBlockSize, WorldBlockSize, j * WorldBlockSize, i * WorldBlockSize, NULL); }
		}
	}
